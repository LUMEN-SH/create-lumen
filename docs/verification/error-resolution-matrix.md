...
| oxlint run returns **empty output** and exit 0 under pipes | oxlint 1.x suppresses its formatted text report on non-TTY stdout | Harness uses `oxfmt`'s sibling: `oxlint . --format=json` and asserts `json.diagnostics.length === 0` |
